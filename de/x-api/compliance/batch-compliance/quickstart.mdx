---
title: Schnellstart
sidebarTitle: Schnellstart
---

import { Button } from '/snippets/de/button.mdx';

<div id="getting-started-with-the-batch-compliance-endpoints">
  ## Erste Schritte mit den Batch-Compliance-Endpunkten
</div>

Die Arbeit mit den Batch-Compliance-Endpunkten umfasst in der Regel fünf Schritte:

1. Erstellen eines Compliance-Jobs
2. Vorbereitung der Liste von Post-IDs oder Benutzer-IDs
3. Hochladen der Liste von Post-IDs oder Benutzer-IDs
4. Prüfen des Status des Compliance-Jobs
5. Herunterladen der Ergebnisse

In diesem Abschnitt zeigen wir, wie Sie jeden dieser Schritte über die Befehlszeile durchführen. Wenn Sie Beispielcode in verschiedenen Programmiersprachen sehen möchten, besuchen Sie bitte unser [GitHub-Repository mit Beispielcode für die X-API v2](https://github.com/xdevplatform/Twitter-API-v2-sample-code).

<div id="prerequisites">
  ### Voraussetzungen
</div>

Um diese Anleitung abzuschließen, benötigen Sie eine Reihe von [Keys und Tokens](/de/resources/fundamentals/authentication), um Ihre Anfrage zu authentifizieren. Sie können diese Keys und Tokens generieren, indem Sie die folgenden Schritte ausführen:

- [Registrieren Sie sich für ein Entwicklerkonto](https://developer.x.com/en/apply-for-access) und lassen Sie es freigeben.
- Erstellen Sie ein [Projekt](/de/resources/fundamentals/projects) und eine zugehörige [Developer-App](/de/resources/fundamentals/developer-apps) im Developer-Portal.
- Navigieren Sie zur Seite „Keys and tokens“ Ihrer App, um die erforderlichen Anmeldeinformationen zu generieren. Speichern Sie alle Anmeldeinformationen an einem sicheren Ort.

<div id="step-one-create-a-compliance-job">
  #### Schritt eins: Einen Compliance-Job erstellen
</div>

Zuerst müssen Sie einen Compliance-Job erstellen und dabei angeben, ob Sie Post-IDs oder User-IDs hochladen (mit dem Parameter type). Optional können Sie Ihrem Job auch einen Namen geben (mit dem Parameter name).

Um diese Anfrage zu autorisieren, müssen Sie die [App-only-Authentifizierung](/de/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token) verwenden. Ersetzen Sie dazu unten $APP\_ACCESS\_TOKEN durch Ihr [App Access Token](/de/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token), das Sie in Ihrer [X-App](/de/resources/fundamentals/developer-apps) im Developer-Portal generieren können.

`curl --request POST 'https://api.x.com/2/compliance/jobs' \ --header 'Authorization: Bearer $APP_ACCESS_TOKEN' \ --header 'Content-Type: application/json' \ --data-raw '{"type": "tweets"}'`

Wenn Ihr API-Aufruf erfolgreich ist, erhalten Sie eine Antwort ähnlich der folgenden:

```
{
   "data": {
       "download_expires_at": "2021-08-18T19:42:55.000Z",
       "status": "created",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/submission/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "created_at": "2021-08-11T19:42:55.000Z",
       "resumable": false,
       "id": "1425543269983784962",
       "type": "tweets",
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/delivery/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "upload_expires_at": "2021-08-11T19:57:55.000Z"
   }
}
```

Notieren Sie sich die Werte der Felder upload\_url, download\_url und id; diese benötigen Sie in den folgenden Schritten.

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### Schritt zwei: Erstellen Sie eine Liste von Post-IDs oder User-IDs
</div>

Erstellen Sie eine Textdatei mit Post-IDs oder User-IDs, wobei jede Zeile eine Post-ID oder eine User-ID enthält. Der Inhalt der Textdatei kann in etwa so aussehen:

1417856744319971329

1415457498803232770

1415348607813832708

1413515358766452738

. . .

. . .

<Note>
  **Hinweis**: Die oben genannte Datei kann entweder Post-IDs oder User-IDs enthalten und darf keine Mischung aus beiden sein.

  Sie können Ihre Datei per **PUT**-Anfrage direkt an diese URL hochladen. Beachten Sie, dass die URL bereits mit einem Authentifizierungs-Token signiert ist, d. h., Sie müssen sich nicht erneut authentifizieren, indem Sie Ihr App Access Token übergeben. Achten Sie darauf, einen Content-Type-Header zu setzen, um anzugeben, dass Sie eine Textdatei hochladen, und ersetzen Sie unten $FILE\_LOCATION durch den Pfad zu Ihrer Datei.
</Note>

```
curl -X PUT  \
 -H "Content-Type: text/plain" \
 --data-binary @$FILE_LOCATION \
"https://storage.googleapis.com/twitter-compliance/customer_test_object_123456_d8ske9.json?X-Goog-Algorithm=\nGOOG4-RSA-SHA256&X-Goog-Credential=example%40example-project.iam.gserviceaccount\n.com%2F20181026%2Fus-central-1%2Fstorage%2Fgoog4_request&X-Goog-Date=20181026T18\n1309Z&X-Goog-Expires=900&X-Goog-SignedHeaders=host&X-Goog-Signature=247a2aa45f16\\n2ea7abedc098d2eb14a7"
```

Ein Statuscode 200 zeigt an, dass der Upload erfolgreich war.

<div id="step-three-optional-check-your-job-status">
  #### Schritt drei (optional): Überprüfen Sie den Status Ihres Jobs
</div>

Große Uploads können einige Zeit in Anspruch nehmen. Sie können den Status über den Content-Compliance-Job-Endpunkt abfragen, indem Sie die Job-ID angeben, die Sie im ersten Schritt erhalten haben.

Stellen Sie erneut sicher, dass Sie den unten stehenden $APP\_ACCESS\_TOKEN durch Ihren App-Access-Token ersetzen und $ID durch Ihre Job-ID aus dem ersten Schritt.

`curl --request GET 'https://api.x.com/2/compliance/jobs/$ID' \ --header 'Authorization: Bearer $APP_ACCESS_TOKEN'`

Die Antwort enthält Informationen zum Job.

```
{
   "data": {
       "upload_expires_at": "2021-08-05T01:50:11.000Z",
       "type": "tweets",
       "resumable": false,
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1423095206576984067/delivery/1202726487847104512_1423095206576984067?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210805%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210805T013511Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "id": "1423095206576984067",
       "status": "expired",
       "created_at": "2021-08-05T01:35:11.000Z",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1423095206576984067/submission/1202726487847104512_1423095206576984067?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210805%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210805T013511Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "download_expires_at": "2021-08-12T01:35:11.000Z"
   }
}
```

Ein vollständiger Status bedeutet, dass die Ergebnisse zum Herunterladen bereitstehen. Hinweis: Die anderen möglichen Statuswerte sind created, complete, in\_progress, failed und expired.

<div id="step-four-download-the-results">
  #### Schritt vier: Ergebnisse herunterladen
</div>

Sobald Sie den Abschlussstatus eines Jobs erhalten, können Sie die Compliance-Ergebnisse über die im Feld download\_url angegebene URL herunterladen (ebenfalls im ersten Schritt erzeugt). Diese URL ist bereits mit einem Authentifizierungstoken signiert, sodass Sie sich nicht erneut authentifizieren müssen, indem Sie Ihr App Access Token übergeben.

```
curl --request GET \
'https://storage.googleapis.com/twttr-tweet-compliance/1423047488781488129/delivery/1202726487847104512_1423047488781488129?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210804%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210804T222534Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature='
```

Das Ergebnis enthält eine Reihe von JSON-Objekten (ein Objekt pro Zeile). Jedes Objekt enthält eine Post- oder Benutzer-ID, das Erstellungsdatum des Posts oder Benutzers (nützlich, um Posts nach Datum zu finden), die erforderliche Aktion, den Grund für die Compliance-Aktion sowie deren Datum:

```
{"id":"1265324480517361664","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"deleted"}
{"id":"1263926741774581761","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"protected"}
{"id":"1265324480517361669","action":"delete","created_at":"2019-10-29T17:02:47.000Z","redacted_at":"2020-07-29T17:02:47.000Z","reason":"suspended"}
```

Ihr Code kann jede JSON-Zeile parsen, um die Post- oder Benutzer-ID zu ermitteln und die Posts und Benutzer mit diesen IDs aus Ihrem Datensatz zu löschen, um konform zu bleiben. Wenn es kein entsprechendes JSON-Objekt für eine von Ihnen hochgeladene ID gibt, können Sie davon ausgehen, dass diese ID konform ist.

**Hinweis**: Nicht alle Compliance-Ereignisse enthalten das Feld redacted\_at.