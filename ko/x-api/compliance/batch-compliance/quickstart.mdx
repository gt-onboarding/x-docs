...
<div id="getting-started-with-the-batch-compliance-endpoints">
  ## 배치 컴플라이언스 엔드포인트 시작하기
</div>

배치 컴플라이언스 엔드포인트 작업은 일반적으로 다음의 5단계를 포함합니다.
...
이 섹션에서는 명령줄을 사용해 각 단계를 수행하는 방법을 다룹니다. 다양한 프로그래밍 언어의 샘플 코드를 보려면 [X API v2 샘플 코드 GitHub 저장소](https://github.com/xdevplatform/Twitter-API-v2-sample-code)를 방문하세요.
...
먼저 컴플라이언스 작업을 생성하고 `type` 매개변수를 사용해 게시물 ID를 업로드할지 사용자 ID를 업로드할지 지정해야 합니다. 선택적으로 `name` 매개변수를 사용해 작업에 이름을 지정할 수도 있습니다.

이 요청을 인증하려면 [앱 전용](/ko/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token)을 사용해야 합니다. 이를 위해 아래의 $APP\_ACCESS\_TOKEN을 개발자 포털의 [X App](/ko/resources/fundamentals/developer-apps)에서 생성할 수 있는 [App Access Token](/ko/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token)으로 교체하세요.
...
API 호출이 성공하면 다음과 유사한 응답이 반환됩니다:
...
upload\_url, download\_url, 그리고 id 필드의 값을 메모해 두세요. 다음 단계에서 필요합니다.

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### 2단계: 게시물 ID 또는 사용자 ID 목록 준비
</div>

각 줄에 게시물 ID 또는 사용자 ID를 하나씩 포함한 텍스트 파일을 만듭니다. 텍스트 파일의 내용은 다음과 같을 수 있습니다:
...
<Note>
  **참고**: 위 파일에는 게시물 ID 또는 사용자 ID 중 하나만 포함되어야 하며, 두 종류를 섞을 수 없습니다.

  이 URL로 **PUT** 요청을 보내 파일을 직접 업로드할 수 있습니다. 이 URL에는 이미 인증 토큰이 포함되어 있으므로 App Access Token을 다시 전달해 인증할 필요가 없습니다. 텍스트 파일 업로드임을 알리기 위해 Content-Type 헤더를 포함하고, 아래의 $FILE\_LOCATION을 파일 경로로 바꾸세요.
</Note>
...
상태 코드 200은 업로드가 성공했음을 의미합니다.

<div id="step-three-optional-check-your-job-status">
  #### 3단계(선택): 작업 상태 확인
</div>

대용량 업로드는 처리에 시간이 걸릴 수 있습니다. 1단계에서 받은 작업 ID를 지정하여 컴플라이언스 작업 엔드포인트에서 상태를 조회할 수 있습니다.

아래의 $APP\_ACCESS\_TOKEN을 앱 액세스 토큰으로, $ID를 1단계에서 받은 작업 ID로 바꿔 주세요.
...
완료 상태는 결과를 다운로드할 준비가 되었음을 의미합니다. 참고: 상태의 다른 값으로는 created, complete, in\_progress, failed, expired가 있습니다.

<div id="step-four-download-the-results">
  #### 4단계: 결과 다운로드
</div>

작업 완료 상태를 받으면 `download_url` 필드에 표시된 URL(1단계에서 생성됨)에서 컴플라이언스 결과를 다운로드할 수 있습니다. 이 URL에는 이미 인증 토큰이 서명되어 있으므로 App Access Token을 다시 전달해 인증할 필요가 없습니다.
...
결과에는 JSON 객체 집합이 포함됩니다(한 줄당 하나의 객체). 각 객체에는 Post 또는 사용자 ID, Post 또는 사용자의 생성 날짜(날짜별로 정리된 Post를 찾는 데 유용), 필요한 조치, 컴플라이언스 조치 사유, 그리고 해당 날짜가 포함됩니다:
...
코드는 각 JSON 라인을 파싱해 Post 또는 사용자 ID를 찾고, 해당 ID의 Post와 사용자를 데이터 세트에서 삭제해 컴플라이언스를 준수할 수 있습니다. 업로드한 ID에 해당하는 JSON 객체가 없으면 해당 ID는 컴플라이언스를 충족하는 것으로 간주할 수 있습니다.

**참고**: 모든 컴플라이언스 이벤트에 `redacted_at` 필드가 포함되지는 않습니다.