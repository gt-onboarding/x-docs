...
정렬은 대부분의 Ads API 컬렉션 엔드포인트(객체 목록을 반환하는 엔드포인트)에서 지원됩니다. 엔드포인트에 따라 정렬에 사용할 수 있는 다양한 매개변수가 제공됩니다. 이러한 매개변수로 반환되는 필드 중 대부분(ID 및 ENUM 제외)은 정렬할 수 있습니다.
...
API 요청에 `sort_by=[attribute name]-[asc/desc]`를 간단히 추가하면 됩니다. 여기서 attribute는 GET 요청의 JSON에 포함되어 반환되는 유효한 속성입니다.

예를 들어 이름을 알파벳 순으로 정렬하려면 `?name-asc`, 역순으로 정렬하려면 `?name-desc`를 사용할 수 있습니다.

<div id="going-deeper">
  ## 더 깊이 알아보기
</div>

아래는 `bid_amount_local_micro`를 기준으로 모든 라인 아이템을 내림차순으로 반환하는 샘플 API 요청입니다:
