...
<div id="getting-started-with-the-batch-compliance-endpoints">
  ## バッチ型コンプライアンスエンドポイントの始め方
</div>

バッチ型コンプライアンスエンドポイントの利用は、一般的に次の5つの手順で行います。

1. コンプライアンスジョブの作成
2. 投稿IDまたはユーザーIDのリストの準備
3. 投稿IDまたはユーザーIDのリストのアップロード
4. コンプライアンスジョブのステータスの確認
5. 結果のダウンロード

このセクションでは、コマンドラインを使ってこれらの各手順を順に実行する方法を学びます。各種プログラミング言語のサンプルコードをご覧になりたい場合は、[X API v2 sample code GitHub repository](https://github.com/xdevplatform/Twitter-API-v2-sample-code)をご参照ください。

<div id="prerequisites">
  ### 前提条件
</div>

このガイドを完了するには、リクエストを認証するための一式の[keys and tokens](/ja/resources/fundamentals/authentication)が必要です。以下の手順に従って、これらの keys と tokens を生成できます。

- [開発者アカウントに申し込み](https://developer.x.com/en/apply-for-access)し、承認を受けます。
...
<div id="step-one-create-a-compliance-job">
  #### ステップ1：コンプライアンスジョブを作成する
</div>

まず、コンプライアンスジョブを作成し、Post ID とユーザー ID のどちらをアップロードするか（type パラメータを使用）を指定します。必要に応じて、ジョブ名（name パラメータ）も付与できます。

このリクエストを認可するには、[App only](/ja/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token) を使用します。実行時は、以下の $APP\_ACCESS\_TOKEN を、開発者ポータルの [X App](/ja/resources/fundamentals/developer-apps) で生成できる [App Access Token](/ja/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token) に置き換えてください。
...
upload\_url、download\_url、id フィールドの値を控えておいてください。次の手順で使用します。

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### ステップ2：投稿IDまたはユーザーIDのリストを用意する
</div>

投稿IDまたはユーザーIDを含むテキストファイルを作成し、各行に1つの投稿IDまたはユーザーIDを記載します。テキストファイルの内容は次のようになります：
...
<Note>
  **注記**: 上記のファイルには投稿IDかユーザーIDのどちらかのみを含め、両方を混在させることはできません。

  このURLに対して **PUT** リクエストでファイルを直接アップロードできます。URLにはすでに認証トークンで署名が付与されているため、App Access Token を再度渡して認証する必要はありません。テキストファイルをアップロードすることを示すために Content-Type ヘッダーを指定し、以下の $FILE\_LOCATION をファイルのパスに置き換えてください。
</Note>
...
ステータスコード 200 は、アップロードが成功したことを示します。

<div id="step-three-optional-check-your-job-status">
  #### ステップ3（任意）：ジョブのステータスを確認する
</div>

大きなアップロードは処理に時間がかかる場合があります。最初のステップで受け取ったジョブ ID を指定して、コンテンツ・コンプライアンスのジョブ用エンドポイントからステータス更新をリクエストできます。

再度、以下の $APP\_ACCESS\_TOKEN をアプリのアクセス・トークンに、$ID を最初のステップのジョブ ID にそれぞれ置き換えてください。
...
complete のステータスは、結果をダウンロードできる状態であることを意味します。注: ほかのステータス値には、created、complete、in\_progress、failed、expired があります

<div id="step-four-download-the-results">
  #### ステップ4：結果をダウンロードする
</div>

ジョブの完了ステータスを受け取ったら、`download_url` フィールドに示されている URL（ステップ1でも生成）からコンプライアンス結果をダウンロードできます。この URL はすでに認証トークンで署名済みのため、App Access Token を再度渡して認証する必要はありません。
...
結果には JSON オブジェクトの集合（1 行につき 1 オブジェクト）が含まれます。各オブジェクトには、Post またはユーザーの ID、Post またはユーザーの作成日時（日時で Post を整理するのに有用）、必要なアクション、コンプライアンスアクションの理由、およびその実行日時が含まれます。
...
コードでは各 JSON 行を解析して Post またはユーザー ID を特定し、該当する ID の Post やユーザーをデータセットから削除することでコンプライアンスを維持できます。アップロードした ID に対応する JSON オブジェクトが存在しない場合は、その ID はコンプライアンスに準拠していると見なせます。

**注**: すべてのコンプライアンスイベントに `redacted_at` フィールドが含まれるとは限りません。